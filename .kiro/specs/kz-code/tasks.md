...
- [x] 21. 統合テストの実装
- [x] 21.1 認証フローの統合テスト
  - サインアップ → ログイン → ダッシュボードのフローをテスト（18テスト）
  - Supabase Auth をモックして AuthService の全メソッドを検証
  - 入力バリデーション、エラーハンドリング、フロー統合テストを実装
  - _Requirements: 1.1, 1.2_

- [x] 21.2 レッスン完了フローの統合テスト
  - レッスン表示 → 完了記録 → 進捗更新のフローをテスト（19テスト）
  - Supabase DB をモックして ProgressService の全メソッドを検証
  - 冪等性、RLSポリシー違反、進捗率算出のテストを実装
  - _Requirements: 4.4, 6.1_

- [x] 21.3 AI チャットフローの統合テスト
  - 質問送信 → ストリーミング応答 → 履歴保存のフローをテスト（14テスト）
  - Claude API と Supabase をモックして ChatService・ClaudeService を検証
  - ストリーミング応答の読み取り、会話履歴の管理、エンドツーエンドフローを実装
  - _Requirements: 5.1, 5.2, 5.3_

- [x] 22. E2E テストの実装
- [x] 22.1 ログインフローの E2E テスト
  - Playwright でログインフローをテスト（メール・パスワード入力 → ダッシュボード表示）
  - 有効/無効な認証情報、バリデーション、リダイレクトのテストを実装
  - _Requirements: 1.2_

- [x] 22.2 オンボーディングフローの E2E テスト
  - Playwright でオンボーディングフローをテスト（初回ログイン → オンボーディング完了 → ダッシュボード）
  - 学習目標・経験レベルの選択、バリデーション、リダイレクトのテストを実装
  - _Requirements: 2.1, 2.2_

- [x] 22.3 レッスン閲覧フローの E2E テスト
  - Playwright でレッスン閲覧フローをテスト（コース選択 → レッスン表示 → MDX レンダリング → 完了記録）
  - ナビゲーション、コンテンツ表示、完了機能のテストを実装
  - _Requirements: 4.2, 4.4_

- [x] 22.4 AI チャットフローの E2E テスト
  - Playwright で AI チャットフローをテスト（質問入力 → ストリーミング表示 → 履歴表示）
  - メッセージ送信、ストリーミング応答、会話履歴、エラーハンドリングのテストを実装
