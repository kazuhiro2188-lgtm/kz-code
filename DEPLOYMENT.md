...
3. **環境変数を追加**
   ...
   **必須環境変数:**
   - **Key**: `NEXT_PUBLIC_SUPABASE_URL`
     - **Value**: SupabaseプロジェクトのURL（例: `https://your-project.supabase.co`）
     - **Environment**: Production, Preview, Development すべてにチェック
   
   - **Key**: `NEXT_PUBLIC_SUPABASE_ANON_KEY`
     - **Value**: Supabaseの匿名キー（Supabaseダッシュボード → Settings → API → anon/public key）
     - **Environment**: Production, Preview, Development すべてにチェック
   
   - **Key**: `ANTHROPIC_API_KEY`
     - **Value**: Anthropic Claude APIキー
     - **Environment**: Production, Preview, Development すべてにチェック
   
   - **Key**: `NEXT_PUBLIC_SITE_URL`
     - **Value**: 本番環境のURL（例: `https://your-app.vercel.app`）
     - **Environment**: Production のみにチェック

   **オプション環境変数:**
   - **Key**: `DISABLE_AUTH`
     - **Value**: `false`（本番環境では通常設定しない）
     - **Environment**: Development のみにチェック（開発環境用）
   
   - **Key**: `ADMIN_BYPASS_KEY`
     - **Value**: 強力なランダム文字列（例: `openssl rand -hex 32`で生成）
     - **Environment**: Production のみにチェック（管理者バイパス用）

4. **環境変数の適用**
   - 各環境変数を追加後、「Save」をクリック
   - 環境変数を変更した場合は、再デプロイが必要です
   - 「Redeploy」ボタンから再デプロイを実行

#### 環境変数の確認方法

- Vercelダッシュボード → Settings → Environment Variables で一覧を確認
- 各環境変数の右側に「⋯」メニューから編集・削除が可能
- 「Value」列はセキュリティのため、一部のみ表示されます

#### 注意事項

- `NEXT_PUBLIC_*`で始まる環境変数はブラウザに公開されます（機密情報を含めない）
- `ANTHROPIC_API_KEY`などの機密情報は`NEXT_PUBLIC_`プレフィックスを付けない
